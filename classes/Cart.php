...
namespace Cargie\Booking\Classes;
...
class Cart
{
    ...
    public function add($id, $price = 0, $quantity = 1, array $attributes = [])
    {
        $cart = $this->getCart();
        $id = (int) $id;
        $quantity = (int) $quantity;
        if ($id > 0 && $quantity > 0) {
            $itemId = $this->generateItemId($id, $attributes);
     ...
     * @return boolean
     */
    public function generateItemId($productId, $attributes)
    {
        ksort($attributes);
